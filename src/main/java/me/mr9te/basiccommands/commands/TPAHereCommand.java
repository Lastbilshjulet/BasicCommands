...
public record TPAHereCommand(BasicCommands plugin) implements CommandExecutor {
        ...
        if (sender instanceof Player player) {
            if (args.length > 0) {
                ...
                if (!players.contains(target) || target == null) {
                    ...
                    target.sendMessage(ChatColor.DARK_GREEN + player.getDisplayName() + ChatColor.GREEN + " wants to teleport you to them, type /tpaccept to accept or /tpdeny to deny. ");
                    player.sendMessage(ChatColor.GREEN + "Request sent to " + ChatColor.DARK_GREEN + target.getDisplayName() + ChatColor.GREEN + ". ");
                    plugin.addRequest(player, target, TypeOfTP.TPAHERE);
                }
            } else {
